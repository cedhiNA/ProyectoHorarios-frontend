      ...
      <Grid container spacing={6}>
        <Grid item xs={12}>
          ...
          <Stack spacing={2.5} alignItems="center">
            <Avatar alt="Avatar 1" src={user?.sexo === 'F'? avatarFemale : avatarMale} sx={{ width: 124, height: 124, border: '1px dashed' }} />
            <Stack spacing={0.5} alignItems="center">
              <Typography variant="h5">{user?.nombre}</Typography>
              <Typography variant="h5">{user?.apellidos}</Typography>
              <Typography color="secondary">{user?.categoria}</Typography>
            </Stack>
          </Stack>
        </Grid>
