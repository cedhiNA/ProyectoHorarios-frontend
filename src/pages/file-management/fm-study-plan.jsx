...
import { getModeloCurso, getModeloFullCurso } from '../../api/gestionArchivos';
...
export default function DropzoneStudyPlan() {
  ...
  const getModeloCursoHandler = async () => {
    await getModeloCurso().then(() => {
      openSnackbar({
        open: true,
  ...
  const getModeloFullCursoHandler = async () => {
    await getModeloFullCurso().then(() => {
      openSnackbar({
        open: true,
        message: 'Documento generado con éxito',
        anchorOrigin: { vertical: 'top', horizontal: 'right' },
          ...
          secondary={
            <Stack direction="row" alignItems="center" spacing={1.25}>
              <IconButton
                ...
                onClick={getModeloFullCursoHandler}
              >
